...
def get_filters():
    ...
    city = input('Choose City: chicago, new york, or washington:').lower()

    while city not in CITY_DATA.keys():
        city = input('Check the spelling, [chicago, new york city, or washington:]:').lower()
    
    month = input('Which month? [january, february, march, april, may, june] or all?').lower()
    months = ['january', 'february', 'march', 'april', 'may', 'june']
    if month != 'all':
        while month not in months:
            month= input('Check the spelling, [january, february, march, april, may, june] or all').lower()
    ...
    if day != 'all':
        while day not in days:
            day = input('Check spelling: sunday, monday, tuesday, wednesday, thursday, friday, saturday:').lower()
...
def station_stats(df):
    ...
    print('Most most frequent trip: {}'.format(df['combine'].mode()[0]))
...
def user_stats(df):
    ...
    if 'Gender' in df:
        ...
        print('Gender not included in df')
    ...
    if 'Birth Year' in df:
        ...
        print('YEar of Birth not included in df')
...
def raw_data_display(df):
    ...
    display = input("Do you want to view 5 rows of trip data? (yes or no)?").lower()
    if display not in ['yes', 'no']:
        display = input ("Please type (yes or no:)").lower
    elif display == "no":
        print('Done!')
    else:
        current_loc = 0
        while (display =="yes"):
            print(df.iloc[current_loc:current_loc+5])
            current_loc += 5
            display = input("Do you want to view more rows?: (yes or no)?").lower()
